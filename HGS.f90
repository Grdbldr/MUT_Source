...
!        
!        ! faces
!        logical :: FacesCalculated
!        integer :: nface  = 0
!	    integer :: Current_nFaces
...
!          !      open(itmp,file=fname,status='unknown',form='unformatted')
!		        !read(itmp) hgs.nface
!		        !allocate(hgs.FaceNode(hgs.nface,4),hgs.FaceNeighbourElement(hgs.nface,2),hgs.Face_Is(hgs.nface),stat=ialloc)
!		        !call AllocChk(ialloc,'Read 3d grid face arrays')
!		        !hgs.FaceNode = 0 ! automatic initialization
!		        !hgs.FaceNeighbourElement = 0 ! automatic initialization
!		        !hgs.Face_Is=0
!		        !read(itmp) ((hgs.FaceNode(i,j),j=1,4),i=1,hgs.nface)
!		        !read(itmp) ((hgs.FaceNeighbourElement(i,j),j=1,2),i=1,hgs.nface)
...
!!		        !write(itmp) ((hgs.FaceNode(i,j),j=1,4),i=1,hgs.nface)
!!		        !write(itmp) ((hgs.FaceNeighbourElement(i,j),j=1,2),i=1,hgs.nface)
...
!        type (HGSProject) global
!
!	    if(Global.FacesCalculated) return
!
...
!
!	
!	    if(allocated(global.nFaceNodes)) deallocate(global.nFaceNodes,global.iFace)
!	    allocate(global.nFaceNodes(global.mesh.nn),global.iFace(global.mesh.nn,MAX_CNCTS),stat=ialloc)
!	    call AllocChk(ialloc,'calc_3d_faces_block arrays')
!    !	Set nFaceNodes and global.iFace to zero. They will be set below and used later to find faces by node number
!	    global.nFaceNodes(:) =0
!	    global.iFace(:,:) =0
!
!	    global.Current_nFaces=1
...
!			
!			    global.FaceNode(global.nface,1)=global.mesh.in(faceid_block(1,j),i)
!			    global.FaceNode(global.nface,2)=global.mesh.in(faceid_block(2,j),i)
!			    global.FaceNode(global.nface,3)=global.mesh.in(faceid_block(3,j),i)
!			    if(faceid_block(4,j) > 0) then
!				    global.FaceNode(global.nface,4)=global.mesh.in(faceid_block(4,j),i)
!			    end if
!
!			    global.FaceNeighbourElement(global.nface,1)=i
!
...
!			    if(global.FaceNode(i1,isrt1(1)) == global.FaceNode(i2,isrt2(1)) .and. global.FaceNode(i1,isrt1(2)) == global.FaceNode(i2,isrt2(2)) .and.  &
!			       global.FaceNode(i1,isrt1(3)) == global.FaceNode(i2,isrt2(3)) .and. global.FaceNode(i1,isrt1(4)) == global.FaceNode(i2,isrt2(4))) then  ! check
!				    dup(i2)=.true.
!				    global.FaceNeighbourElement(i1,2)=global.FaceNeighbourElement(i2,1)
...
!		    if(.not. dup(i)) then 
!			    nface_new=nface_new+1
!			    face_node_temp(nface_new,1) = global.FaceNode(i,1)
!			    face_node_temp(nface_new,2) = global.FaceNode(i,2)
!			    face_node_temp(nface_new,3) = global.FaceNode(i,3)
!			    face_node_temp(nface_new,4) = global.FaceNode(i,4)
!			    face_el_temp(nface_new,1)=global.FaceNeighbourElement(i,1)
!			    face_el_temp(nface_new,2)=global.FaceNeighbourElement(i,2)
!			    face_is_temp(nface_new)=global.Face_Is(i)
!
!			    i1=minval(face_node_temp(nface_new,:),MASK=face_node_temp(nface_new,:).gt.0)
!			    global.nFaceNodes(i1)=global.nFaceNodes(i1)+1
!	 		    !call user_size_check(global.nFaceNodes(i1),user_face_nd,user_face_nd_str)
!			    global.iFace(i1,global.nFaceNodes(i1))=nface_new
!		    end if
!	    end do
!
!	    deallocate(dup)
...
!	    deallocate(global.FaceNode,global.FaceNeighbourElement,global.Face_Is)
!	    allocate(global.FaceNode(global.nface,4),global.FaceNeighbourElement(global.nface,2),global.Face_Is(global.nface),stat=ialloc)
!	    call AllocChk(ialloc,'final face arrays')
!	    global.FaceNode(:,:)=face_node_temp(:,:)
!	    global.FaceNeighbourElement(:,:)=face_el_temp(:,:)
...
!!	Set global.nFaceNodes and global.iFace to zero. They will be set below and used later to find faces by node number
!	global.nFaceNodes(:) =0
!	global.iFace(:,:) =0
!
!	global.Current_nFaces=1
...
!			
!			global.FaceNode(global.nface,1)=global.mesh.in(faceid_prism(1,j),i)
!			global.FaceNode(global.nface,2)=global.mesh.in(faceid_prism(2,j),i)
!			global.FaceNode(global.nface,3)=global.mesh.in(faceid_prism(3,j),i)
!			if(faceid_prism(4,j) > 0) then
!				global.FaceNode(global.nface,4)=global.mesh.in(faceid_prism(4,j),i)
!			end if
!
!			global.FaceNeighbourElement(global.nface,1)=i
!
...
!    
!    do i=1,global.nface
!        eCur=global.FaceNeighbourElement(i,1)
!        eConn=global.FaceNeighbourElement(i,2)
!        if(eConn /= 0) then
...
!	integer :: proposed
!
!	if(proposed > global.Current_nFaces) global.Current_nFaces=proposed
!
!	if(allocated(global.FaceNode)) then
!		deallocate(global.FaceNode, global.FaceNeighbourElement, global.Face_Is)
!	end if
!
!	allocate(global.FaceNode(global.Current_nFaces,4), global.FaceNeighbourElement(global.Current_nFaces,2), global.Face_Is(global.Current_nFaces),stat=ialloc)
!	call AllocChk(ialloc,'initial face arrays')
!    global.FaceNode(:,:)=0
!    global.FaceNeighbourElement(:,:)=0
!	global.Face_Is(:)=0
!
...
!		test(i)=global.FaceNode(i,isrt1(1))  ! store min node number as test
!	end do
!	
...
!
!			if(global.FaceNode(i1,isrt1(2)) > global.FaceNode(i2,isrt2(2))) then  ! swap values
!				itemp=i2
!				indx(i+1)=i1
!				indx(i)=itemp
!
...
!			global.FaceNode(i1,isrt1(2)) == global.FaceNode(i2,isrt2(2))) then 
!
!			if(global.FaceNode(i1,isrt1(3)) > global.FaceNode(i2,isrt2(3))) then  ! swap values
!
!				itemp=i2
!				indx(i+1)=i1
!				indx(i)=itemp
...
!
!		if(test(i1)==test(i2) .and.  &
!			global.FaceNode(i1,isrt1(2)) == global.FaceNode(i2,isrt2(2)) .and. &
!			global.FaceNode(i1,isrt1(3)) == global.FaceNode(i2,isrt2(3))) then 
!
!			if(global.FaceNode(i1,isrt1(4)) > global.FaceNode(i2,isrt2(4))) then  ! swap values
!
!				itemp=i2
!				indx(i+1)=i1
!				indx(i)=itemp
...
!!		    ncount=0
!!		    do j=1,4
!!			    if(hgs.FaceNode(i,j)/=0) then
!!                    if(bcheck(hgs.node_is(hgs.FaceNode(i,j)),chosen)) then
!!				        ncount=ncount+1
!!                    end if
!!                end if
!!            end do
!!        
!!            if(hgs.FaceNode(i,4)/=0 .and. ncount==4) then   ! rectangular fracture element
!!			    hgs.mesh.nef=hgs.mesh.nef+1
!!                hgs.ap(hgs.mesh.nef)=ApDefault
!!                hgs.ifrac_id_elem(hgs.mesh.nef)=hgs.nzones_fprop
!!    
!!                do j=1,4
!!				    hgs.mesh.inf(j,hgs.mesh.nef)=hgs.FaceNode(i,j)
!! 				    call set(hgs.node_is(hgs.mesh.inf(j,hgs.mesh.nef)),a_frac)
!!			    end do
!!                
!!            else if (hgs.FaceNode(i,4)==0 .and. ncount==3) then   ! triangular fracture element    
!!			    hgs.mesh.nef=hgs.mesh.nef+1
!!                hgs.ap(hgs.mesh.nef)=ApDefault
!!                hgs.ifrac_id_elem(hgs.mesh.nef)=hgs.nzones_fprop
!!                do j=1,3
