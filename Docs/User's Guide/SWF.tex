...
Depression and obstruction storage heights are written to the \texttt{.swf} package.

By default, \mfus\ reconstructs SWF velocity from solved face fluxes using the same smoothed flow depth as the SWF conductance (\texttt{SWF\_THIK}). The following instruction selects the original HydroGeoSphere Manning reconstruction from the water-surface gradient, also using that flow-equation depth (obstruction remains a relative conductivity). \mut\ writes \texttt{OPTIONS} / \texttt{ORIGINAL\_SWF\_VELOCITY} / \texttt{END} in the VEL package; a \texttt{usgs\_1} build that recognizes that option is required.

\ins{original swf velocity calculation}
    {
        No arguments.  When present, SWF velocity in the \texttt{.SWF.VEL} binary uses the original Manning reconstruction rather than face-flux velocity. \gwf\ and \cln\ velocity are unchanged.
    }
...
By default, the instruction {\sf postprocess existing modflow model} writes hydraulic heads at the last output time to a {\tt csv} file called {\tt SWF\_FinalHeads.csv}. This file can be used to start a subsequent \mfus\ model simulation by first activating the \swf\ domain then using this instruction:
...
The following example uses the instruction to start a simulation with \swf\ heads from a previous run:

            \includegraphics[width=\textwidth]{MUT_InitialSWFHead.png}
            
The instruction {\sf postprocess existing modflow model}  also writes heads at all output times to a \tecplot-formatted output file, and earlier times can also be used to start a subsequent \mfus\ model simulation by first activating the \swf\ domain then using this instruction:

\ins{swf initial head from tecplot file}
    {
        \squish
        \begin{enumerate}
            \item \str{FName}  The hydraulic head solution in the file \str{FName} will be assigned as the initial head in the currently active domain.
...
This file can be created using \tecplot, as discussed for the \gwf\ domain on Page~\pageref{page:TecplotWriteData}.
