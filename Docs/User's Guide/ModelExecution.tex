...
\ins{postprocess existing modflow model}
    {
        ...
        \end{enumerate}
         ...
         \begin{itemize}
            ...
            \item  Reads Darcy and average-linear velocity binaries written by the USG \texttt{VEL} package and writes {\tt \_posto.modflow.GWF.Velocity.tecplot.dat} with sequential 1-D arrays \texttt{Darcy Vx/Vy/Vz} and \texttt{Average Linear Vx/Vy/Vz} for Tecplot vector mapping; analogous files are written for \cln\ and \swf\ when those domains exist. Velocity Tecplot requires a USG-Transport build that supports NAM file type \texttt{VEL}.
...
After \texttt{mut \_build} has been run in a model folder, the Python tool \texttt{mut\_document.py} writes a dossier PDF under a \texttt{Docs} subfolder named after the parent directory (the simulation name).  The PDF summarises the problem comments in \texttt{\_build.mut}, referenced data and include files, mesh construction, materials, initial and boundary conditions, and---when \texttt{usgs\_1} and \texttt{mut \_post} products are present---run status, volumetric-budget mass-balance checks, and result imagery.

From a command prompt rooted at the model folder (after deploying MUT, including the \texttt{mut\_document} folder next to \texttt{mut.exe}):
\begin{verbatim}
...
\begin{itemize}
  ...
  \item \texttt{Docs/layouts/\textit{domain}\_Mesh.lay}, \texttt{\textit{domain}\_Variables.lay}, \texttt{\textit{domain}\_scatter.lay}, \texttt{\textit{domain}\_Results.lay}, and \texttt{\textit{domain}\_Observations.lay} for each of GWF, SWF, and CLN when present (Tecplot~360 EX 2018 does not accept multi-page \texttt{\$\!PAGECONTROL} commands).  The mesh layout uses zone colours, named legends, mesh lines, and lighting.  The variables layout has one frame per non-$xyz$ build field.  The scatter layout has one frame per build-time point file (\texttt{\_CELLS}, \texttt{\_CHD}, \texttt{\_RCH}, \texttt{\_SWBC}, \texttt{\_DRN}, \texttt{\_WEL}, \texttt{\_OBS\_scatter}); spheres of size 1.0, coloured by the optional fourth column when present.  The results layout has one frame per post-processed field, plus velocity and on GWF the volume budget.  The observations layout has square frames in two rows (head on top, saturation or depth on the bottom), sites left-to-right alphabetically; extra columns may sit off the initial view.  \texttt{GWF.lay} / \texttt{SWF.lay} / \texttt{CLN.lay} are copies of the mesh layout.
