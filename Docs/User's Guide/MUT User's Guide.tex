...
\newcommand{\str} [1] {$\overline {S\_#1}$}
\newcommand{\rnum} [1] {$\overline {R\_#1}$}
\newcommand{\inum} [1] {$\overline {I\_#1}$}
...
\begin{titlepage}
    ...
    \vfill
    {\bfseries\Huge
        Modflow User Tools (MUT) Version 1.33\\
         User's Guide\\
    }
    \vspace{.3in}
...
\chapter{\mfus\ Execution and Post-Processing}\input{ModelExecution}

\chapter{Model Verification}\input{ModelVerification}
    ...
    \section{3D Fully-coupled Groundwater-Surface Water Flow: Abdul's Experiment}\input{3D_Abduls_Problem}

\chapter{Illustrative Example} \input{IllustrativeExample}
