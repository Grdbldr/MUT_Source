...
During the build, \mut\ always writes the USG \texttt{VEL} package (\texttt{Modflow.vel}), binary velocity units for each active domain (\texttt{.GWF.VEL}/{\tt .GWF.VELLin}, and {\tt .CLN.VEL}/{\tt .CLN.VELLin} and/or {\tt .SWF.VEL} when those domains exist), and the corresponding \texttt{SAVE DARCY/LINEAR VELOCITY} lines in the OC file. No special \mut\ instruction is required. A USG-Transport executable that recognizes NAM file type \texttt{VEL} is required for the solver to fill those binaries; {\tt mut \_post} then converts them to Tecplot velocity files.
...
\begin{itemize}
    ...
    \item \tecplot\ output files are indicated by the suffix \texttt{.tecplot.szplt} (TecIO SZL). ASCII \texttt{.tecplot.dat} can be requested instead.
    ...
    ASCII \tecplot\ files can be requested instead of the default SZL binary:

        \ins{write ascii tecplot output}
            {Finite-element mesh and results files are written as ASCII \texttt{.tecplot.dat} instead of TecIO SZL \texttt{.tecplot.szplt}. Scatter, custom-label and volume-budget files remain ASCII in either case. Place this instruction before \texttt{build modflow usg} or, for \texttt{mut \_post}, before \texttt{postprocess existing modflow model}.}

        \ins{no model documentation}
            {No arguments. By default, after a successful build \mut\ runs the deployed \texttt{mut\_document.py} tool with \texttt{--skip-export} (Tecplot PNG export is skipped) and writes \texttt{Docs/\textit{folder}.pdf} plus layouts. This instruction skips that step. Place it before \texttt{build modflow usg} or, for \texttt{mut \_post}, before \texttt{postprocess existing modflow model}. Missing Python, the script, or \texttt{pdflatex} produces a warning; the \mut\ run still ends with \texttt{Normal exit}. See Chapter~\ref{chapter:ModelExecution}.}
