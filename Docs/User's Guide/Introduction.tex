...
\begin{itemize}
    ...
    \item We will introduce the use of \qgis\footnote{\qgis\ is a geographic information system (GIS) software that is free and open-source. It supports viewing, editing, printing, and analysis of geospatial data in a range of data formats.} software to view and process project data consisting of physiography (e.g.\ DEM's, often provided as raster-formatted TIF files) and model boundaries (e.g.\ often provide as shapefiles).
...
Changes made to \mut\  since version 1.34 are listed in Appendix~\ref{chapter:modifications}
...
\begin{description}
    ...
    \item[Appendix~\ref{appendix:release_verification}]\textbf{Release verification} How to re-run the verification models with \texttt{mut\_verify}, plus comparison of the current \mut\ release against the previous-version baseline, including the \cln/\swf\ CHD start-to-end head ramps that change \texttt{3\_0\_SWF\_CHD} and \texttt{3\_1\_CLN\_for\_SWF}.
        \end{description}
