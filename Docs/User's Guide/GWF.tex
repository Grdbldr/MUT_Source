...
The \gb\ nodal property file uses a legacy binary file format. You can develop your own ascii input files and read elevation data from them using this instruction:
...
\ins{choose cells from xyz list}
    {This subtask reads $xyz$ coordinates from a list.
    \begin{enumerate}
        \item \rnum{x1} [$L$], \rnum{y1} [$L$], \rnum{z1} [$L$]  An $xyz$ coordinate triplet.
    \end{enumerate}
    ...
    {\Large \sf end choose cells from xyz list}
    }
...
Shown below is the output echoed to the screen and \texttt{\_buildo.eco} file for the example \texttt{1\_VSF\_Column}: \begin{verbatim}
    ...
    chosen cells use gwf material number
        ...
        Porosity:              0.43000         DIMENSIONLESS (for average linear velocity)
...
The following section describes instructions used to assign values to individual materials properties for the current cell selection.  When using these instruction, {\em you must be careful to supply the values in the unit system defined for the model}. 

By default, \mfus\ simulates variably-saturated flow in the \gwf\ domain.  This instruction can be used to switch to a confined, saturated flow solution:

\ins{saturated flow}
{\squish}

This instruction is used to define the horizontal hydraulic conductivity, \texttt{Kh}:
...
By default, the instruction {\sf postprocess existing modflow model} writes hydraulic heads at the last output time to a {\tt csv} file called {\tt GWF\_FinalHeads.csv}. This file can be used to start a subsequent \mfus\ model simulation by first activating the \gwf\ domain then using this instruction:

\ins{initial head from csv file}
    {
        \squish
        \begin{enumerate}
            \item \str{FName}  The hydraulic head solution in the file \str{FName} will be assigned as the initial head in the currently active domain.
        \end{enumerate}
    }

The following example uses the instruction to start a simulation with \gwf\ heads from a previous run:

            \includegraphics[width=\textwidth]{MUT_InitialGWFHead.png}

The instruction {\sf postprocess existing modflow model}  also writes heads at all output times to a \tecplot-formatted output file, and earlier times can also be used to start a subsequent \mfus\ model simulation by first activating the \gwf\ domain then using this instruction:

\ins{gwf initial head from tecplot file}
    {
        \squish
        \begin{enumerate}
            \item \str{FName}  The hydraulic head solution in the file \str{FName} will be assigned as the initial head in the currently active domain.
...
\label{page:TecplotWriteData}
This file can be created using \tecplot, as shown in the example below:

            \includegraphics[width=\textwidth]{MUT_IHeadFromTecplot.png}

Note the following:
\begin{itemize}
    \item The post-processed data file {\tt \_posto.modflow.GWF.tecplot.dat} for the example {\tt 6\_Abdul\_Prism\_Cell} has been loaded into \tecplot.
    \item The {\sf Data Set Information} dialogue is open showing {\sf Zone 7:GWF} corresponds to {\sf Solution Time} 1500.
    \item The {\sf Write Data} dialogue is open and configured to write {\sf Field data} using {\sf Format: Block} from {\sf Zone(s): 7:GWF, Variable(s): 8: GWF Heads}.
    \item The data is being written to a file called {\tt Head\_1500.GWF.tecplot.dat}, shown below.
\end{itemize}

            \includegraphics[width=\textwidth]{MUT_IHeadFromTecplot_2.png}

 For earlier output times, the post-processed heads can be By default, the instruction {\sf postprocess existing modflow model} writes hydraulic heads at the last output time to a {\tt csv} file called {\tt GWF\_FinalHeads.csv}. This file can be used to start a subsequent \mfus\ model simulation by first activating the \gwf\ domain then using this instruction:

To assign an initial head equal to the local ground surface elevation use this instruction:

\ins{gwf initial head equals surface elevation}
     {For each column of cells in the \gwf\ domain, each cell in the column is assigned an initial head equal to the top elevation (i.e. ground surface elevation) of the cell at the top of the column.
    }

This is commonly used as an initial guess when starting a simulation with no previous head solution.

For the \cln\ domain, an equivalent instruction is also available:
\ins{cln initial head equals surface elevation}
    {For each \cln\ cell, the initial head is set to the land-surface elevation of the corresponding \gwf\ column (top of the shallowest layer in that column), using the same column convention as \texttt{gwf initial head equals surface elevation}. If \cln--\gwf\ connection data is already available, the maximum of those column surface elevations is used for that cell; otherwise the \gwf\ top-layer cell nearest in XY is used.}
...
To assign a linearly varying head that is a function of saturation versus depth, use this instruction:
\ins{gwf initial head from depth-saturation table}
    {
    \squish
    \begin{enumerate}
    \item \rnum{dpth(1)} [$L$], \rnum{sat(1)} [$L$]  First depth \rnum{dpth(1)}, saturation \rnum{sat(1)} pair.
    \item \rnum{dpth(2)} [$L$], \rnum{sat(2)} [$L$]  Second depth \rnum{dpth(2)}, saturation \rnum{sat(2)} pair.
    \item \textbf{...}\\
     \hspace*{-.27in}\rule{0.in}{.24in} n. \rnum{dpth(n)} [$L$], \rnum{sat(n)} [$L$]  nth depth \rnum{dpth(n)}, saturation \rnum{sat(n)} pair.
    \end{enumerate}

     For each column of cells in the \gwf\ domain, each local cell in the column is assigned a depth calculated from the top elevation (i.e. ground surface elevation) of the cell at the top of the column minus the local cell elevation.  A saturation is then looked up from the depth-saturation table.  A pressure head is than interpolated from the pressure-saturation table defined for the \gwf\ material.

     An end instruction is required to stop the subtask e.g.:

    {\Large \sf end gwf initial head from depth-saturation table}
    }

This was used to generate an initial head for the example simulation \texttt{9\_Smith\_Woolhiser} as shown here:

            \includegraphics[width=\textwidth]{MUT_IheadDepthSat.png}
...
A constant head boundary condition fixes the head at a \gwf\ cell at a given value, allowing water to flow into or out of the \gwf\ model domain depending on surrounding conditions.
...
\ins{chd zone name}
    {
        \squish
        \begin{enumerate}
        \item \rnum{ZoneName}   Budget label for this CHD group (maximum 16 characters; underscores recommended).
        \end{enumerate}
          The label is applied to cells that receive the next \ins{gwf constant head} assignment. USGs\_1 prints separate IN/OUT lines for each named zone, and still prints the aggregate \texttt{CONSTANT HEAD} total.
    }
