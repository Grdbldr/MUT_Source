...
\ins{cln from xyz list}
    {This subtask creates a \cln\ domain from a list of $xyz$ coordinates.
    \begin{enumerate}
    ...
    {\Large \sf end cln from xyz list}
    }
...
\begin{verbatim}
    chosen zones use cln material number
    	...
    	Infill Porosity:             1.0000     DIMENSIONLESS (for average linear velocity)
\end{verbatim}

An optional trailing \texttt{InfillPorosity} column may be added to \texttt{CLN.csv}. When less than 1, average linear velocity in the conduit equals Darcy velocity divided by the infill porosity.

\ins{cln skin conductivity}
    {
        \squish
        \begin{enumerate}
        \item \rnum{FSKIN} [$T^{-1}$]  CLN-to-GWF skin/leakance term applied to the chosen \cln\ cells.
        \end{enumerate}
        The value \rnum{FSKIN} is assigned to all currently chosen \cln\ cells.
    }

For CLN-GWF connections, \mut\ now writes a default \texttt{FSKIN} value to the \texttt{.CLN} file based on connected \gwf\ hydraulic conductivity:
\begin{itemize}
\item If a \cln\ cell connects to one \gwf\ cell, default \texttt{FSKIN} is that \gwf\ cell's \texttt{Kh}.
\item If a \cln\ cell connects to multiple \gwf\ cells, default \texttt{FSKIN} is the highest connected \texttt{Kh}.
\item If \ins{cln skin conductivity} is used, the user-assigned value overrides this default for the selected \cln\ cells.
\end{itemize}

\subsubsection{General Cross-Section Geometry (Tabular)} \index{\cln\ Domain ! material properties ! general cross-section}
In addition to \texttt{Circular} and \texttt{Rectangular} cross-sections, \mut\ supports a \texttt{General} cross-section type for \cln\ domains.
This geometry is defined by a table of depth versus cross-sectional properties, and is written to the \texttt{.CLN} file using the MODFLOW-USG \texttt{GENERAL\_SEC} option.

When \texttt{Geometry=General} is used in \texttt{CLN.csv}, an additional column can be provided at the end of the line specifying a table file name.
The table file should contain one data row per depth, with four columns:
\begin{verbatim}
    depth   area   wetted_perimeter   top_width
\end{verbatim}
Blank lines and lines beginning with \texttt{\#} are ignored.
All general cross-section tables used in a model must contain the same number of rows.
...
By default, the instruction {\sf postprocess existing modflow model} writes hydraulic heads at the last output time to a {\tt csv} file called {\tt CLN\_FinalHeads.csv}. This file can be used to start a subsequent \mfus\ model simulation by first activating the \cln\ domain then using this instruction:
...
An alternative initial-head instruction is:
...
The following example uses the instruction to start a simulation with \cln\ heads from a previous run:

            \includegraphics[width=\textwidth]{MUT_InitialCLNHead.png}
